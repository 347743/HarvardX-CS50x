...
def buy():
    if request.method == "GET":
        ...
        print(f"Received symbol: {symbol}, shares: {shares}")
        ...
        user_id = session["user_id"]
        user_cash_db = db.execute("SELECT cash FROM users WHERE id = ?", user_id)[0]["cash"]

        # Check if user can afford the purchase
        if user_cash_db < total_cost:
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, timestamp) VALUES (?, ?, ?, ?, ?)",
                   user_id, symbol, shares, price_per_share, datetime.now())

        # Update or insert stock into user's portfolio
        portfolio = db.execute(
            "SELECT shares FROM portfolio WHERE user_id = ? AND symbol = ?", user_id, symbol)
        if portfolio:
            # If stock already in portfolio, update the share count
            db.execute("UPDATE portfolio SET shares = shares + ? WHERE user_id = ? AND symbol = ?",
                       shares, user_id, symbol)
        else:
            # If stock not in portfolio, insert new entry
            db.execute("INSERT INTO portfolio (user_id, symbol, shares) VALUES (?, ?, ?)",
                       user_id, symbol, shares)

        # Redirect to home after successful transaction
        return redirect("/")  # Redirect to home after successful transaction
...
def login():
    ...
    # If user is already logged in, redirect to /buy
    ...
    if request.method == "POST":
        ...
        # Redirect user to the buy page after successful login
        return redirect("/buy")
